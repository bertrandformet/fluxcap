...
- **Partage externe** : ⚠️ implémenté via l'API Web Share native du navigateur (`navigator.share`), qui ouvre le menu de partage natif macOS/iOS — au lieu du raccourci Apple Shortcuts unique synchronisé via iCloud initialement prévu. Repli automatique sur copie presse-papiers si l'API n'est pas disponible
