...
- **Capture externe entrante** : ✅ raccourci Apple Shortcuts (menu Partager, authentifié via une clé API dédiée) et favori Chrome/bookmarklet (authentifié via la session déjà connectée, sans clé exposée) — voir [raccourci-partage.md](./raccourci-partage.md). L'écran de confirmation avant enregistrement (voir "Fix sécurité" plus haut) propose aussi de choisir des tags de domaine (mêmes chips à bascule que "+ tag" ailleurs), pour ne pas avoir à retourner classer la note après coup
